    ...
    async def get_result(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            while status == 3:
                ...
                if status == 4:
                    break
                await asyncio.sleep(15)  # 轮讯秒数，存在频率限制，低数值太快会被拒绝，高数值需要反复计算signa。差不多5~30秒轮询一次合适。
...
# 导入的时候，【导这个函数】
async def xf_asr(upload_file_name, appid, secret_key):
