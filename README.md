...
```bash
git clone https://github.com/erlingmijiang/smart_notes.git
cd smart_notes
uv sync
```
...
1. 未来也许会把 ASR 的功能扒掉，这样就能拿来把文档转换成笔记了
2. 未来也许会抽空拿 AI 撸个前端界面出来，但目前还没有好的构思（其实我也不懂）
3. 未来也许会把音频下载和语音识别拆成异步，但目前来说稍微复杂一点的异步我就会翻车hhh
4. 调整模块运行顺序，避免等待音频识别完后再进行提示词模板选择（已实现）
